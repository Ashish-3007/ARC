...
import { searchMovies, getMoviesByGenre, MOCK_MOVIES } from "@/lib/mock-data"
...
export default function SearchPage() {
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      if (debouncedQuery.trim()) {
        searchResults = searchMovies(debouncedQuery)
      } else if (selectedGenres.length > 0) {
        searchResults = selectedGenres.flatMap((genre) => getMoviesByGenre(genre))
        // Remove duplicates
        searchResults = searchResults.filter((movie, index, self) => index === self.findIndex((m) => m.id === movie.id))
      } else {
        searchResults = MOCK_MOVIES
      }

      // Apply genre filters
      if (selectedGenres.length > 0 && debouncedQuery.trim()) {
        searchResults = searchResults.filter((movie) => selectedGenres.some((genre) => movie.genres?.includes(genre)))
      }
  ...
  return (
    <PageLayout title="Discover Movies" subtitle="Search and filter through our collection">
      <div className="space-y-8">
                ...
                >
                  {SORT_OPTIONS.map((option) => (
                    <DropdownMenuItem
                      ...
                      className={sortBy === option.value ? "bg-purple-600 text-white" : ""}
                    >
                      {option.label}
                    </DropdownMenuItem>
                  ))}
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <p className="text-gray-400">{isLoading ? "Searching..." : `${results.length} movies found`}</p>
          </div>
